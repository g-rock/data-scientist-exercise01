...
With 82 percent accuracy a random forest classifier will determine whether an individual makes more than, or less than fifty thousand dollars in 1996. A few of the most important drivers of salary classification are relationship status (husband, wife, unmarried), age, education level, and capital gain (figure 2). The final model was chosen over a few other high performing models. The deciding factors were
  ...
  3. High true positive rate and low false positive rate (FNR) - we want the model to perform well when classifying      the positive class at various false positives rate (figure 3). In this example, a false positive is predicting that someone makes a salary less than 50k when in fact they make more.
