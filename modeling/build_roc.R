...
plot_roc = function(pred1, pred2, pred3, pred4){
  par(mfrow=c(1,1))
  ...
  sens4 = performance(pred4, "sens", "spec")
  ...
  plot(perf4, add = T, col='Black')
  legend(0.7,
         0.3,
         c(sprintf("%3.3f glm, cutoff = 0.50",auc1@y.values[[1]]),
           sprintf("%3.3f glm, cutoff = 0.45", auc2@y.values[[1]]),
           sprintf("%3.3f rf, cutoff = 0.50",auc3@y.values[[1]]),
           sprintf("%3.3f rf, cutoff = 0.45",auc4@y.values[[1]])),
         lty=c(1,1),
         lwd=c(2.5,2.5),
         c(col="red",
           col="blue",
